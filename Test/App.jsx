...
const SecurityApp = () => {
  ...
  const [messages, setMessages] = useState({
    "Secure Contact A": [{ id: 1, text: "Welcome to SecureChat!", sender: "system" }],
    "Project Group": [{ id: 1, text: "Welcome to Project Group!", sender: "system" }]
  });
  ...
  const settingsData = {
    profile: { title: "Profile Settings", content: ["Change Avatar", "Update Username", "Encryption Key Management"] },
    decoy: { title: "Decoy Mode", content: ["Enable Duress Password", "Fake Chat History", "Auto-wipe on failure"] },
    proxy: { title: "Proxy Settings", content: ["SOCKS5 Configuration", "Tor Routing", "HTTP Proxy"] },
    customServer: { title: "Custom Server", content: ["Server URL: https://", "Port: 443", "TLS Certificate Pinning"] }
  };

  // --- MOBILE TOP BAR ---
  const TopBar = () => (
    <div style={{ display: 'flex', justifyContent: 'space-around', borderBottom: '1px solid #333', padding: '10px 0' }}>
      <button onClick={() => { setView('chat'); setActiveChat(null); }} style={styles.btnStyle}>💬 Chat</button>
      <button onClick={() => setView('settings')} style={styles.btnStyle}>⚙️ Settings</button>
      <button onClick={() => setView('addFriends')} style={styles.btnStyle}>➕ Friends</button>
    </div>
  );
  ...
  const ChatListMobile = () => (
    <div style={{ padding: '20px' }}>
      ...
      {chats.map(chat => (
        <div
          key={chat.name}
          style={{ ...styles.chatThumbStyle, display: 'flex', alignItems: 'center', gap: '10px' }}
  ...
  const ChatBoxMobile = () => (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh' }}>
      <button onClick={() => setActiveChat(null)} style={{ ...styles.sendBtnStyle, margin: '10px' }}>⬅ Back</button>
          ...
          <div style={styles.main}>
            {view === 'chat' && (
              <>
                ...
                <div style={styles.chatList}>
                  ...
                  <button style={styles.addFriendBtnStyle} onClick={() => setView('addFriends')}>+ Add Friends</button>
                </div>
            ...
            {view === 'addFriends' && (
              <div style={styles.addFriendView}>
                <h2>Add Friends</h2>
                <p>Friend adding UI here</p>
                <button onClick={() => setView('chat')} style={{ marginTop: '20px', background: 'none', color: 'gray', border: 'none', cursor: 'pointer' }}>
                  Back to Chat
                </button>
              </div>
            )}
...
const styles = {
  ...
  btnStyle: { fontSize: '16px', background: 'none', border: 'none', color: 'white', cursor: 'pointer', marginBottom: '10px' },
