...
import { ChatMessage } from "../Home/types"; // Fixed import
...
import {
  ...
  decryptData
} from "../../services/SafeStorage"; // Fixed import
...
interface VaultItem {
  ...
  iv?: string; // Made optional as SafeStorage handles IV in payload
  timestamp: number;
}
...
export const SecureChatWindow: React.FC = () => {
  ...
  const checkLockStatus = async () => {
    const email = ChatClient.userEmail; // Fixed usage
    if (!email) return;

    try {
      const key = await AccountService.getStorageKey(email, "app_lock_pin");
      const pin = await getKeyFromSecureStorage(key);
      if (pin) {
        setIsSetup(true);
      } else {
        setIsSetup(false);
  ...
  const setupVault = async (pin: string) => {
    ...
    try {
      const key = await AccountService.getStorageKey(email, "app_lock_pin");
      await setKeyFromSecureStorage(key, pin);
      setIsSetup(true);
      setIsUnlocked(true);
    } catch (e) {
      setVaultError("Setup failed");
    }
  };
  ...
  const handleSendMessage = () => {
    ...
    const newMsg: ChatMessage = { // Using ChatMessage
  ...
  const handleViewItem = async (item: VaultItem) => {
    try {
      const decrypted = await decryptData(item.encrypted_data); // Using decryptData
      if (item.type === 'file') {
        // Handle file view
  ...
  if (!isUnlocked) {
    return (
      <div style={{ width: "100%", height: "100%", position: "relative", background: colors.background.primary }}>
        <AppLockScreen
          mode="input"
  ...
  return (
    <SecureContainer>
      <SecureHeader>
        <HeaderLeft>
          ...
          <TitleContainer>
            ...
            <StatusText>End-to-End Encrypted • Self-Destructing Messages</StatusText>
          </TitleContainer>
        </HeaderLeft>

        <HeaderActions>
          <VaultButton isActive={isVaultOpen} onClick={() => setIsVaultOpen(!isVaultOpen)}>
            {isVaultOpen ? <Key size={18} /> : <Lock size={18} />}
            <span>Vault</span>
          </VaultButton>
          <button style={{ background: "none", border: "none", color: colors.text.primary, cursor: "pointer" }}>
      ...
      <div style={{ position: 'relative', flex: 1, display: 'flex', overflow: 'hidden' }}>
        <MessageList onClick={() => setIsVaultOpen(false)}>
          <div style={{
            ...
            <span>Messages in this chat are end-to-end encrypted and not stored on any server.</span>
          </div>
              ...
              items.map(item => (
                <ScVaultItem key={item.id} onClick={() => handleViewItem(item)}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <span style={{ color: 'white', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <InputArea>
        <button style={{ background: "none", border: "none", color: colors.text.secondary, cursor: "pointer" }}>
          <Paperclip size={20} />
        </button>
        <TextInput
          value={inputText}
      ...
      {viewingItem && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
            ...
            <button onClick={closeView} style={{ background: 'none', border: 'none', color: 'white', fontSize: '24px', cursor: 'pointer' }}>✕</button>
          </div>

          <div style={{ flex: 1, overflow: 'auto', background: '#111', padding: '20px', borderRadius: '8px' }}>
            {viewingItem.type === 'password' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <code style={{ fontSize: '1.2rem', color: '#fbbf24' }}>{viewingItem.value}</code>
                <button onClick={() => navigator.clipboard.writeText(viewingItem.value)} style={{ background: 'none', border: 'none', color: '#aaa' }}>
                  <Copy size={20} />
                </button>
              </div>
            )}
            {viewingItem.type === 'note' && (
              <p style={{ color: '#ddd', whiteSpace: 'pre-wrap' }}>{viewingItem.value}</p>
            )}
            {viewingItem.type === 'file' && viewingItem.mimeType?.startsWith('image/') && (
              <img src={viewingItem.contentUrl} alt="Decrypted" style={{ maxWidth: '100%' }} />
            )}
            {viewingItem.type === 'file' && !viewingItem.mimeType?.startsWith('image/') && (
              <div style={{ textAlign: 'center', padding: '40px' }}>
                <p style={{ color: 'white' }}>File decrypted successfully.</p>
                <a href={viewingItem.contentUrl} download={viewingItem.title} style={{ color: '#60a5fa' }}>Download File</a>
              </div>
            )}
          </div>
        </div>
